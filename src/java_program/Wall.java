...
public class Wall {//Class with wall

    double width,height;//2 instance varible

    public Wall(){}//1 construstor with no parameter & no argument
    public Wall(double width,double height){//second constructor has parameters width and height of type double
        this.width=width;
        this.height=height;
      if (width<0){
          this.width=0;}
    ...
    public double setWidth(double width) {
        if (width >= 0){this.width=0;}return width;

    } public double setHeight(double height ){
        if (height >= 0){
           this.height=0;}return height;}
